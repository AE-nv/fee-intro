...
export default function CartPage() {
  ...
  // The basket only stores ids and quantities, so the menu is needed to show
  // names and prices. While it loads there is nothing to look them up in.
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-8 lg:px-8 lg:py-10">
        ...
        {isLoading && (
          <div className="mt-6 rounded-2xl border border-stone-200 bg-white p-8">
            <Spinner>Je zakje wordt opgehaald</Spinner>
          </div>
        )}

        {isError && (
          <div className="mt-6 rounded-2xl border border-stone-200 bg-white p-8">
            <p className="font-black">Er ging iets mis.</p>
            <p className="mt-1 text-sm text-stone-500">
              We konden je zakje niet ophalen. Probeer het later opnieuw.
            </p>
          </div>
        )}
        ...
        {!!sandwiches &&
          (lines.length === 0 ? (
            ...
            <>
              <p className="mt-1 text-sm text-stone-500">
                {itemCount} {itemCount === 1 ? "broodje" : "broodjes"} ·
                levering op kantoor
              </p>
